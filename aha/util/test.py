    ...
    if args.sparse:
        ...
        for app in args.app:
            ...
            # Use voyager gold pre-supplied by the user
                ...
                # TODO: For now assume only one output for voyager gold
                assert len(outputs) == 1, "Voyager gold path currently supports only one output."
                output_file_name, _ = os.path.splitext(outputs[0]["datafile"])
