...
def dispatch(args, extra_args=None):
    ...
    if args.sparse:
        ...
        if sparse_comp is None:
            sparse_comp = f"/aha/garnet/SPARSE_TESTS/{testname}_5/GLB_DIR/{testname}_combined_seed_5/"

        outputs = ["0", "1", "2", "3"]
        for output in outputs:
            # This is where we do the fallback comparison...
            # First get gold matrix from the output...
            gold_matrix = numpy.load(f"{sparse_comp}/output_gold_{output}.npy")
            name_line = None
            ...
            print(just_out_files_sim)
            for file__ in just_out_files_sim:
                convert_aha_glb_output_file(f"/aha/garnet/tests/test_app/{file__}", "/aha/garnet/SPARSE_TESTS/", suffix=f"_{output}")
