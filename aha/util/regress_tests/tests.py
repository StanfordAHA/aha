...
class Tests:
    ...
    def configs_mu():
            ...
            return vars().copy()
    configs['mu'] = configs_mu()



    def configs_xxx():    configs['xxx'] = configs_xxx()
    def configs_xxx():    configs['xxx'] = configs_xxx()
            return vars().copy()
            return vars().copy()
    ...
    # ------------------------------------------------------------------------------------
    # FIXME/TODO can replace this mechanism with a simple
    # "if 'kernel' in test and not 'kernel0' in test ... right?
    # ------------------------------------------------------------------------------------
    skip_cgra_map = [
        "resnet18-conv2d_mx_default_16 -> zircon_nop_post_conv5_x_kernel1_RV_E64_MB",
        "resnet18-submodule_17 -> zircon_residual_relu_fp_post_conv5_x_kernel1_RV_E64_MB",
