...
def test_dense_app(test, width, height, env_parameters, extra_args, layer=None, dense_only=False, use_fp=False, using_matrix_unit=False, mu_datawidth=16, num_fabric_cols_removed=0, mu_oc_0=32, dense_ready_valid=False, E64_mode_on=False, E64_multi_bank_mode_on=False, behavioral_MU=False, mu_test="", skip_cgra_map=False, skip_cgra_pnr=False):
...
def dispatch(args, extra_args=None):
    ...
    # For these tests, we skip the CGRA mapping to save time. The assumption is that the collateral was generated by a prior test. This means certain tests must be run in order.
    skip_cgra_map_tests = [
    ...
    # For these tests, we skip the CGRA place and route to save time. The assumption is that the collateral was generated by a prior test. This means certain tests must be run in order.
    skip_cgra_pnr_tests = [
        "resnet18-conv2d_mx_default_16 -> zircon_nop_post_conv5_x_kernel1_RV_E64_MB",
        "resnet18-submodule_16 -> zircon_residual_relu_fp_post_conv5_x_kernel1_RV_E64_MB",
        "resnet18-submodule_16 -> zircon_residual_relu_fp_post_conv5_x_kernel2_RV_E64_MB",
...
#         # assert False, 'debuggin'
    ...
    #         feature_support_check(test, E64_mode_on, E64_multi_bank_mode_on)
    #         t0, t1, t2 = test_dense_app(test, width, height, args.env_parameters, extra_args, layer=layer,
    #                         using_matrix_unit=using_matrix_unit, mu_datawidth=mu_datawidth, num_fabric_cols_removed=num_fabric_cols_removed, mu_oc_0=mu_oc_0,
    ...
    # EOF
    # diff tmp2 tmp4
    # < E64_multi_bank_mode_on=E64_multi_bank_mode_on,                  skip_cgra_map=skip_cgra_map, skip_cgra_pnr=skip_cgra_pnr)
    # > E64_multi_bank_mode_on=E64_multi_bank_mode_on, mu_test=mu_test, skip_cgra_map=skip_cgra_map, skip_cgra_pnr=skip_cgra_pnr)
    # 
    # <         info.append([unparsed_name + "_glb", t0 + t1 + t2, t0, t1, t2])
    # >         info.append([unparsed_name + "_MU_ext", t0 + t1 + t2, t0, t1, t2])
...
#         feature_support_check(test, E64_mode_on, E64_multi_bank_mode_on)
#         t0, t1, t2 = test_dense_app(test, width, height, args.env_parameters, extra_args, layer=layer, use_fp=True,
#                         using_matrix_unit=using_matrix_unit, mu_datawidth=mu_datawidth, num_fabric_cols_removed=num_fabric_cols_removed, mu_oc_0=mu_oc_0,
#                         dense_ready_valid=dense_ready_valid, E64_mode_on=E64_mode_on, E64_multi_bank_mode_on=E64_multi_bank_mode_on, mu_test=mu_test, skip_cgra_map=skip_cgra_map, skip_cgra_pnr=skip_cgra_pnr)
#         info.append([unparsed_name + "_MU_ext", t0 + t1 + t2, t0, t1, t2])
# EOF
# diff tmp2 tmp5
# < bank_mode_on=E64_multi_bank_mode_on,                  skip_cgra_map=skip_cgra_map, skip_cgra_pnr=skip_cgra_pnr)
# > bank_mode_on=E64_multi_bank_mode_on, mu_test=mu_test, skip_cgra_map=skip_cgra_map, skip_cgra_pnr=skip_cgra_pnr)
# 
# <         info.append([unparsed_name + "_glb", t0 + t1 + t2, t0, t1, t2])
# >         info.append([unparsed_name + "_MU_ext", t0 + t1 + t2, t0, t1, t2])
    ...
    for tdict in tlist:
        ...
        t0, t1, t2 = \
            test_dense_app(test, width, height, args.env_parameters, extra_args,
                           use_fp=use_fp, layer=layer,
    ...
    if args.include_no_zircon_tests:
        ...
        for test in glb_tests:
            ...
            t0, t1, t2 = test_dense_app(test, width, height, args.env_parameters, extra_args, dense_only=False, using_matrix_unit=False, num_fabric_cols_removed=0,
                        dense_ready_valid=False, E64_mode_on=False, skip_cgra_map=skip_cgra_map, skip_cgra_pnr=skip_cgra_pnr)
            info.append([test + "_glb", t0 + t1 + t2, t0, t1, t2])
        ...
        for test in resnet_tests:
            ...
            if "residual" in test:
                t0, t1, t2 = test_dense_app("apps/resnet_residual", width, height, args.env_parameters, extra_args, layer=test,
                            dense_only=False, using_matrix_unit=False, num_fabric_cols_removed=0, dense_ready_valid=False, E64_mode_on=False, skip_cgra_map=skip_cgra_map, skip_cgra_pnr=skip_cgra_pnr)
                info.append([test + "_glb", t0 + t1 + t2, t0, t1, t2])
            else:
